...
<form id="pagerForm" method="post" action="/ssadmin/informationVentureArticleList.html">
...
<div class="pageHeader">
    <form onsubmit="return navTabSearch(this);"
          action="/ssadmin/informationVentureArticleList.html" method="post" enctype="multipart/form-data">
        <div class="searchBar">
...
<div class="pageContent">
    <div class="panelBar">
        <ul class="toolBar">
            <shiro:hasPermission name="/ssadmin/saveInformationArticle.html">
                <li><a class="add"
                       href="/ssadmin/goInformationArticleJSP.html?url=ssadmin/addInformationVentureArticle"
            ...
            <shiro:hasPermission name="/ssadmin/updateInformationArticle.html">
                <li><a class="edit"
                       href="/ssadmin/goInformationArticleJSP.html?url=ssadmin/updateInformationVentureArticle&id={article_id}"
            ...
            <shiro:hasPermission name="ssadmin/commentList.html">
                <li>
                    <a height="600" width="900" target="dialog" href="ssadmin/commentList.html?articleId={article_id}" rel="commentList" class="edit"><span>评论列表</span></a>
                </li>
            </shiro:hasPermission>
        </ul>
    </div>
    <table class="table" width="100%" layoutH="138">
        ...
        <tr>
            ...
            <th width="60" orderField="capital"
                    <c:if test='${param.orderField == "capital" }'> class="${param.orderDirection}"  </c:if>>资金</th>
            <th width="60" orderField="financingRound"
                    <c:if test='${param.orderField == "financingRound" }'> class="${param.orderDirection}"  </c:if>>轮次</th>
            <th width="60" orderField="roundTime"
                    <c:if test='${param.orderField == "roundTime" }'> class="${param.orderDirection}"  </c:if>>融资时间</th>
        </tr>
        </thead>
        <tbody>
        <c:forEach items="${articleList}" var="informationArticle" varStatus="num">
            <tr target="article_id" rel="${informationArticle.id}">
                ...
                <td>${informationArticle.capital}</td>
                <td>${informationArticle.financingRound}</td>
                <td><fmt:formatDate value="${informationArticle.roundTime}" pattern="yyyy-MM-dd"></fmt:formatDate></td>
            </tr>
        </c:forEach>
        </tbody>
    </table>
