...
<script type="text/javascript">
	$(function() {
		$('#tradefeeReport').highcharts(
				{
					chart : {},
					title : {
						text : '${coinType}交易手续费统计表,区间总数：${total}'
					},
					xAxis : {
						categories : ${key}
					},
...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);"
		action="/ssadmin/tradefeeReport.html" method="post">
		<div class="searchBar">
			...
			<table class="searchContent">
				<tr>
					<td>开始日期： <input type="text" name="startDate" class="date"
						...
						<input type="hidden" name="url" value="ssadmin/tradefeeReport" />
					</td>
					<td>结束日期： <input type="text" name="endDate" class="date"
						readonly="true" value="${endDate }" /><font color="red">*</font>
					</td>
					<td>虚拟币类型： <select id="selectId" type="combox" name="symbol" onchange="loadTradeFeeReport()">
							<option value="0">全部</option>
							<c:forEach items="${allType}" var="type">
								<c:if test="${type.fid == symbol}">
				...
				<ul>
					<li><div class="buttonActive">
							<div class="buttonContent">
								<button class="reloadTradeFeeReport" type="submit">查询</button>
							</div>
						</div></li>
				</ul>
			</div>
			<script>
                function loadTradeFeeReport() {
                    $(".reloadTradeFeeReport").click();
                }
			</script>
		</div>
	</form>
