...
<form id="pagerForm" method="post" action="ssadmin/entrustPlanList.html">
...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);"
		action="ssadmin/entrustPlanList.html" method="post">
		<div class="searchBar">
...
<div class="pageContent">
	...
	<table class="table" width="120%" layoutH="138">
		...
		<tbody>
			<c:forEach items="${entrustplanList}" var="entrustPlan"
				varStatus="num">
				<tr target="sid_user" rel="${entrustPlan.fid}">
					<td>${num.index +1}</td>
					<td>${entrustPlan.fuser.floginName}</td>
					<td>${entrustPlan.fuser.fnickName}</td>
					<td>${entrustPlan.fuser.frealName}</td>
					<td>${entrustPlan.fvirtualcointype.fname}</td>
					<td>${entrustPlan.ftype_s}</td>
					<td>${entrustPlan.fstatus_s}</td>
					<td>${entrustPlan.fprize}</td>
					<td>${entrustPlan.fcount}</td>
					<td>${entrustPlan.famount}</td>
					<td>${entrustPlan.fcreateTime}</td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
