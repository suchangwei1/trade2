...
<form id="pagerForm" method="post" action="ssadmin/otcOrderList.html">
	...
	<input
		...
		type="hidden" name="endDate" value="${endDate}" /><input type="hidden"
...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);"
		  action="ssadmin/otcOrderList.html" method="post">
		<div class="searchBar">

			<table class="searchContent">
				<tr>
					<td>挂单ID：<input type="text" name="id"
									value="${id}" size="10" placeholder="挂单ID"/></td>
	...
	<div class="panelBar">
		<ul class="toolBar">
			<shiro:hasPermission name="ssadmin/otccheck.html">
				<li><a class="icon" href="ssadmin/otcOrderExport.html"
	...
	<table class="table" width="100%" layoutH="138">
		...
		<c:forEach items="${list}" var="item" varStatus="num">
			<tr target="sid_user" rel="${item.id}">
				<td>${item.id}</td>
				<td>${item.fvirtualcointype.fShortName}</td>
				<td>${item.fuser.frealName}</td>
				<td>${item.type == 1 ? "卖出": "买入"}</td>
				<td><fmt:formatNumber value="${item.price}" pattern="##.##"/></td>
				<td><fmt:formatNumber value="${item.amount}" pattern="##.##"/></td>
				<td><fmt:formatNumber value="${item.minAmount}" pattern="##.##"/> - <fmt:formatNumber value="${item.maxAmount}" pattern="##.##"/></td>
				<td><fmt:formatNumber value="${item.successAmount}" pattern="##.##"/></td>
				<td><fmt:formatNumber value="${item.frozenAmount}" pattern="##.##"/></td>
				<td>${item.pay_s}</td>
				<td>${item.status_s}</td>
