		...
		<div class="searchBar">
			<table class="searchContent">
				<tr>
					...
					<td>放款状态： <select type="combox" name="status2">
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
              <li><a class="add"
					href="ssadmin/lendentrustlogList.html?type=1&uid={sid_user}"
					height="500" width="900" target="dialog" rel="lendentrustlogList"><span>查看放款记录</span>
			  </a></li>
		</ul>
	</div>
...
<%-- 				<th width="40" orderField="fvirtualcointype.fname"
				...
				<%-- <th width="40" orderField="fisBuyInsurance"
					<c:if test='${param.orderField == "fisBuyInsurance" }'> class="${param.orderDirection}"  </c:if>>是否买保险</th>
				 --%>
				...
				<th width="40" orderField="fstatus2"
					<c:if test='${param.orderField == "fstatus2" }'> class="${param.orderDirection}"  </c:if>>收款状态</th>
		...
		<tbody>
			<c:forEach items="${lendentrustList}" var="lendentrust"
				...
				<tr target="sid_user" rel="${lendentrust.fid}">
					...
					<%-- <td>${lendentrust.fisBuyInsurance}</td> --%>
