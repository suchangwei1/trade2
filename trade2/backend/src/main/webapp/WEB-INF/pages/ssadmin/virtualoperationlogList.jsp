...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<shiro:hasPermission name="ssadmin/saveVirtualOperationLog.html">
				<li><a class="add"
			...
			<shiro:hasPermission name="ssadmin/exportVirtualOperationlogList.html">
				<li><a class="icon"
					href="ssadmin/exportVirtualOperationlogList.html"
					target="dwzExport" title="确定要导出充值列表吗?"><span>导出充值列表</span> </a></li>
			</shiro:hasPermission>
		</ul>
	</div>
		...
		<tbody>
			<c:forEach items="${virtualoperationlogList}"
				...
				<tr target="sid_user" rel="${virtualoperationlog.fid}">
					...
					<td>${virtualoperationlog.fuser.femail}</td>
					<td>${virtualoperationlog.fuser.ftelephone}</td>
					...
					<td>${virtualoperationlog.fcreateTime}</td>
                     <td>${empty virtualoperationlog.fcreator.fname ? "":virtualoperationlog.fcreator.fname}</td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
