...
<form id="pagerForm" method="post"
	action="ssadmin/virtualCapitalInTableSum.html">
...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);"
		action="ssadmin/virtualCapitalInTableSum.html" method="post">
		<div class="searchBar">
...
<shiro:hasPermission name="ssadmin/exportVirtualCapitalInTableSum.html">
			<li><a class="icon" href="ssadmin/exportVirtualCapitalInTableSum.html"
	...
	<table class="table" width="100%" layoutH="138">
		...
		<tr>
			...
			<%--<th width="30" orderField="ftelephone"
					<c:if test='${param.orderField == "fTelephone" }'> class="${param.orderDirection}"  </c:if>>会员手机号</th>
			<th width="30" orderField="frealName"
					<c:if test='${param.orderField == "frealName" }'> class="${param.orderDirection}"  </c:if>>会员真实姓名</th>--%>
			...
			<th width="30" orderField="fname"
					<c:if test='${param.orderField == "fname" }'> class="${param.orderDirection}"  </c:if>>虚拟币类型</th>
		...
		<tbody>
		<c:forEach items="${virtualCapitalInTableSum}"
			...
			<%--	<td>${virtualCapitaloperation.ffees}${typeMap[virtualCapitaloperation.feeCoinType + 0].fShortName}</td>--%>
				...
				<td><c:if test=" ${virtualCapitaloperation.fstatus_s}==3"></c:if>充值成功</td>
