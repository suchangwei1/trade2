...
public class FlogDAO extends HibernateDaoSupport {
	...
	public Flog findById(java.lang.Integer id) {
		...
		try {
			Flog instance = (Flog) getSession().get("com.trade.model.Flog", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Flog> findByExample(Flog instance) {
		...
		try {
			List<Flog> results = (List<Flog>) getSession()
					.createCriteria("com.trade.model.Flog").add(create(instance)).list();
	...
	public Flog findLast(String fkey1, String fkey5){
		log.debug("finding Flog instance with property: fkey1 and fkey5");
		try {
			String queryString = "from Flog as model where model.fkey1 = ? and model.fkey5 = ? order by fid desc";
			Query queryObject = getSession().createQuery(queryString);
			queryObject.setParameter(0, fkey1);
			queryObject.setParameter(1, fkey5);
			queryObject.setMaxResults(1);
			List<Flog> list = queryObject.list();
	...
	public List<Flog> findGroupByUser(Integer id){
		...
		log.debug("finding Flog instance with property: fkey1 group by fkey5");
		try {
			String queryString = "from Flog as model where model.fkey1 = ? group by model.fkey5";
			Query queryObject = getSession().createQuery(queryString);
			queryObject.setParameter(0, fkey1);
			List<Flog> list = queryObject.list();
			return list;
		} catch (RuntimeException re) {
			log.error("find by property fkey1 group by fkey5 failed", re);
			throw re;
		}
	}
	...
	public Flog findLast(String fkey5){
		log.debug("finding Flog instance with property: fkey1 and fkey5");
		try {
			String queryString = "from Flog as model where model.fkey5 = ? order by fid desc";
			Query queryObject = getSession().createQuery(queryString);
			queryObject.setParameter(0, fkey5);
			queryObject.setMaxResults(1);
			List<Flog> list = queryObject.list();
			if(CollectionUtils.isEmpty(list)) {
				return null;
