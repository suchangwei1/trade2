...
public class FmessageDAO extends HibernateDaoSupport {
	...
	public Fmessage findById(java.lang.Integer id) {
		...
		try {
			Fmessage instance = (Fmessage) getSession().get("com.trade.model.Fmessage", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public Fmessage findByUser(Fuser user) {
		try {
			String queryString = "from Fmessage as model where model.freceiver =? order by model.fcreateTime desc ";
			Query queryObject = getSession().createQuery(queryString);
			queryObject.setParameter(0, user);
			List<Fmessage> list=queryObject.list();
	...
	public List<Fmessage> findByExample(Fmessage instance) {
		...
		try {
			List<Fmessage> results = (List<Fmessage>) getSession()
					.createCriteria("com.trade.model.Fmessage").add(create(instance)).list();
	...
	public int findFmessageByParamCount(Map<String, Object> param) {
		...
		try {
			...
			queryString.append("fstatus != 3 and");
...
//			queryObject.setCacheable(true);
	...
	public boolean deleteById(final int id, final int userId){
		return getHibernateTemplate().execute(new HibernateCallback<Boolean>() {

			String sql = "update Fmessage set fstatus = 3 where fid = ? and freceiver.fid = ?";
			@Override
			public Boolean doInHibernate(Session session) throws HibernateException, SQLException {
				...
				return rowNum == 0 ? false : true;
			}
		});
	}

	public boolean readMessage(final int id, final int userId){

		return getHibernateTemplate().execute(new HibernateCallback<Boolean>() {

			String sql = "update Fmessage set fstatus = 2 where fid = ? and freceiver.fid = ?";
			@Override
			public Boolean doInHibernate(Session session) throws HibernateException, SQLException {
				Query query = session.createQuery(sql);
				query.setInteger(0, id);
