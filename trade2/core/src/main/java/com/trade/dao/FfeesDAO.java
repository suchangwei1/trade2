...
public class FfeesDAO extends HibernateDaoSupport {
	...
	public Ffees findById(java.lang.Integer id) {
		...
		try {
			Ffees instance = (Ffees) getSession().get("com.trade.model.Ffees", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Ffees> findByExample(Ffees instance) {
		...
		try {
			List<Ffees> results = (List<Ffees>) getSession().createCriteria("com.trade.model.Ffees").add(create(instance)).list();
	...
	// public Ffees findFfee(int virtualCoinType,int level) {
	// log.debug("findFfee all Ffees instances");
	// try {
	// String queryString = "from Ffees where fvirtualcointype.fid=? and
	// flevel=?";
	// Query queryObject = getSession().createQuery(queryString);
	...
	public Ffees findFfee(final int virtualCoinType, final int level) {
		log.debug("findFfee all Ffees instances");
		try {
			return getHibernateTemplate().execute(new HibernateCallback<Ffees>() {

				@Override
				public Ffees doInHibernate(Session session) throws HibernateException, SQLException {
					String queryString = "from Ffees where fvirtualcointype.fid=? and flevel=?";
					Query queryObject = session.createQuery(queryString);
					queryObject.setParameter(0, virtualCoinType);
					queryObject.setParameter(1, level);
					queryObject.setCacheable(true);
					return (Ffees) queryObject.uniqueResult();
				}
			});
			// String queryString = "from Ffees where fvirtualcointype.fid=? and
			// flevel=?";
	...
	public Ffees findFfee(final int virtualCoinType) {
		log.debug("findFfee all Ffees instances");
		try {
			return getHibernateTemplate().execute(new HibernateCallback<Ffees>() {
				...
				@Override
				public Ffees doInHibernate(Session session) throws HibernateException, SQLException {
					String queryString = "from Ffees where fvirtualcointype.fid=?";
					Query queryObject = session.createQuery(queryString);
					queryObject.setParameter(0, virtualCoinType);
					queryObject.setCacheable(true);
					return (Ffees) queryObject.uniqueResult();
