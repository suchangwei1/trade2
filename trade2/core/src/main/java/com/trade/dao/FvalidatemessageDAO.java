...
public class FvalidatemessageDAO extends HibernateDaoSupport {
	...
	public Fvalidatemessage findById(java.lang.Integer id) {
		...
		try {
			Fvalidatemessage instance = (Fvalidatemessage) getSession().get(
					"com.trade.model.Fvalidatemessage", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<Fvalidatemessage> findByExample(Fvalidatemessage instance) {
		...
		try {
			List<Fvalidatemessage> results = (List<Fvalidatemessage>) getSession()
					.createCriteria("com.trade.model.Fvalidatemessage")
	...
	public Fvalidatemessage findOne(String cellphone){
		log.debug("finding one findLastValidatMessage instances");
		try {
			String queryString = "from Fvalidatemessage where fphone =:phone and fstatus =:status order by fcreateTime desc";
			Query queryObject = getSession().createQuery(queryString);
			queryObject.setParameter("phone", cellphone);
			queryObject.setParameter("status", ValidateMessageStatusEnum.Send);
			queryObject.setMaxResults(1) ;
			queryObject.setFirstResult(0) ;
